...
In physics, computation and mathematics, inputs are related to outputs by
compositional processes that compose in series and in parallel. These
processes have a natural graphical syntax — *string diagrams* — and the
`Wolfram`DiagrammaticComputation`` paclet provides a uniform
symbolic representation of them, along with tools for composing,
transforming, rewriting, simplifying and visualising diagrams. A
<code>[Diagram]()</code> is a morphism in a symmetric monoidal category with
typed input and output <code>[Port]()</code>s; everything else in this guide
builds on those two primitives.
