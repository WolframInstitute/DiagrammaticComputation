...
<code>[DiagramReplaceList]()[$d$, $src$ -> $tgt$]</code> returns a list of every possible single rewrite of $d$ replacing a match of the diagram $src$ with $tgt$.

<code>[DiagramReplaceList]()[$d$, $rule$, $n$]</code> returns at most $n$ rewrites.

<code>[DiagramReplaceList]()[$d$, {$rule_1$, $rule_2$, …}]</code> enumerates rewrites for each of the given rules.
