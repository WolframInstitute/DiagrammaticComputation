...
<code>[EraserAnnihilationRule]()[]</code> returns a rewrite rule annihilating two facing <code>[EraserDiagram]()</code>s into nothing.

## Details & Options

- Encodes the interaction-net erase–erase law: an eraser meeting another eraser head-on leaves the empty diagram.
- Options of <code>[AnnihilationRule]()</code> are accepted.
