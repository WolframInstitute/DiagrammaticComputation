...
<code>[AnnihilationRule]()[$d_1$, $d_2$]</code> returns a rewrite rule annihilating the facing diagrams $d_1$ and $d_2$ into identity wires.

<code>[AnnihilationRule]()[$expr_1$, $expr_2$, {$x_1$, …, $x_n$}, {$y_1$, …, $y_n$}]</code> annihilates nodes labelled $expr_1$ and $expr_2$ with legs $x_i$ and $y_i$.
...
<code>[DuplicateAnnihilationRule]()</code> and <code>[EraserAnnihilationRule]()</code> are the special cases for copy and erase nodes.
