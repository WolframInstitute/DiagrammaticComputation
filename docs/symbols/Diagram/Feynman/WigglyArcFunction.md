...
<code>[WigglyArcFunction]()[$shape$]</code> returns a function that renders a curve in the propagator line style $shape$.

<code>[WigglyArcFunction]()[$shape$, $k$]</code> bends the curve into an arc with curvature $k$.

<code>[WigglyArcFunction]()[$shape$, $k$, $arrow$]</code> adds an arrowhead with direction $arrow$ (1 forward, −1 backward, 0 none).
...
- <code>[FeynmanDiagram]()</code> uses this function to draw each propagator according to its field type.
