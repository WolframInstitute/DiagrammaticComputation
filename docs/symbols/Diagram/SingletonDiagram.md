...
<code>[SingletonDiagram]()[$d$]</code> wraps the diagram $d$ as a single opaque node, hiding its internal structure.

<code>[SingletonDiagram]()[$d$, {$i_1$, …}, {$o_1$, …}]</code> wraps $d$ with explicit input and output ports $i_1, …$ and $o_1, …$.
...
- The wrapped diagram preserves its data and behaviour under composition, but <code>[DiagramDecompose]()</code> and surgery tools see it as a single node.
