...
Stack the steps with <code>[ColumnDiagram]()</code> and <code>[RowDiagram]()</code>; matching resource names wire automatically:
...
Each comparison-swap is a <code>[PermutationDiagram]()</code>:
...
Since the network is nothing but wires, <code>[SimplifyDiagram]()</code> contracts it to the overall permutation:
...
In the other direction, <code>[TensorDiagram]()</code> turns a tensor expression into a diagram. A contraction of two arrays over two indices:
...
<code>[ToDiagram]()</code> converts trees and graphs to diagram networks, and the grid renderer can lay them out like an org chart by switching off wires and ports:
