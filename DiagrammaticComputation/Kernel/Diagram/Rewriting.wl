...
DiagramHypergraph[d_Diagram, opts : OptionsPattern[]] := With[{net = SimplifyDiagram[ToDiagramNetwork[d]]}, {f = net["PortFunction"]},
	DiagramHypergraph[DiagramSubdiagrams[net, {1}], f, MapThread[Labeled[#1, Replace[#2, Automatic -> #1]] &, {f /@ net["Ports", True], Catenate @ d["PortLabels"]}], opts]
]
...
DiagramReplaceList[d_Diagram, src_Diagram -> tgt_Diagram, opts : OptionsPattern[]] := Enclose @ Block[{
	...
	nets = Map[
		With[{
			newNet = DiagramNetwork[Join[Delete[diagrams, #["MatchEdgePositions"]], MatchDiagrams[tgtDiagrams, #]]],
