...
$DefaultPortFunction = Function[#["TaggedFullName"]]
...
Diagram[expr : {} | Except[_Association | _Diagram | OptionsPattern[]],
    ...
    Diagram[d,
        "PortArrows" -> {Lookup[annotations, "InputStyle", Inherited], Lookup[annotations, "OutputStyle", Inherited]},
        "PortLabels" -> {Lookup[annotations, "InputLabel", Inherited], Lookup[annotations, "OutputLabel", Inherited]}
    ]
]
...
Diagram /: HoldPattern[Options[d_Diagram ? DiagramQ, args___]] := Options[d["DiagramOptions"], args]

Diagram /: HoldPattern[AbsoluteOptions[d_Diagram ? DiagramQ, args___]] := Options[Lookup[d["AbsoluteData"], "DiagramOptions", {}], args]

Diagram /: OptionValue[d_Diagram ? DiagramQ, opts_ : {},  arg_] := d["AbsoluteOptionValue"[arg], opts]
...
DiagramProduct[ds__Diagram ? DiagramQ, opts : OptionsPattern[]] := With[{subDiagrams = If[#["ProductQ"], Splice[#["SubDiagrams"]], #] & /@ {ds}},
    Diagram[
        ...
        If[ TrueQ[OptionValue["Flat"]],
            {
                "OutputPorts" -> Catenate @ Through[{ds}["FlatOutputPorts"]],
                "InputPorts" -> Catenate @ Through[{ds}["FlatInputPorts"]],
                "PortArrows" -> Catenate /@ Thread[Through[{ds}["PortStyles"]]]
            }
            ,
            {
...
SingletonDiagram[diagram_Diagram, Inherited, args___] :=
    SingletonDiagram[diagram, PortDual /@ diagram["InputPorts"], diagram["OutputPorts"], args]
...
DiagramComposition[ds___Diagram ? DiagramQ, opts : OptionsPattern[]] := With[{
    ...
    ports = If[
        ...
        Join[#["InputOutputPorts", True], {#["PortStyles"], #["PortLabels"]}] & @ ColumnDiagram[Reverse[{ds}], FilterRules[{opts}, Options[ColumnDiagram]]],
        Last @ collectPortsListBy[{PortDual /@ #["InputPorts"], #["OutputPorts"], #["PortStyles"], #["PortLabels"]} & /@ Through[Reverse[{ds}]["Flatten"]], func]
    ]
},
    Diagram[
        "Expression" :> DiagramComposition[##] & @@ subDiagrams,
        "InputPorts" -> PortDual /@ ports[[1]],
        "OutputPorts" -> ports[[2]],
        "PortArrows" -> ports[[3]],
        "PortLabels" -> ports[[4]],
        opts
...
DiagramProp[d_, "AnnotatedInputOutputPorts", args___] :=
    MapThread[MapThread[{p, l, s} |-> If[MatchQ[s, Automatic | None], #, Style[#, s]] & @ If[MatchQ[l, Automatic | None], #, Labeled[#, l]] & @ p, {##}] &, {d["InputOutputPorts", args], d["PortLabels"], d["PortStyles"]}]
...
DiagramProp[diagram_, "Label"] := With[{expr = diagram["Node"]["HoldExpression"]}, {pos = FirstPosition[expr, _Diagram ? DiagramQ]},
    If[MissingQ[pos], expr, FlattenAt[ReplaceAt[expr, d_Diagram :> RuleCondition[d["Label"]], {pos}], {pos}]]
]
...
DiagramProp[d_, "PortFunction"] := With[{f = Lookup[d["DiagramOptions"], "PortFunction", If[d["NetworkQ"], $DefaultNetworkPortFunction, $DefaultPortFunction]]},
    Function[Replace[f[#], x : Except[_HoldForm] :> HoldForm[x]]]
]

DiagramProp[d_, "PortGraph", opts : OptionsPattern[]] := DiagramsPortGraph[d["SubDiagrams"], opts]

DiagramProp[d_, "Graph", opts : OptionsPattern[]] := If[d["NetworkQ"], DiagramsGraph[d["SubDiagrams"], opts, "PortFunction" -> d["PortFunction"]], ToDiagramNetwork[d]["Graph"]]

DiagramProp[d_, "Network", opts : OptionsPattern[]] := ToDiagramNetwork[d, FilterRules[{opts}, Options[ToDiagramNetwork]]]

DiagramProp[d_, "NetGraph", opts : OptionsPattern[]] := DiagramsNetGraph[d["Graph"], FilterRules[{opts, d["DiagramOptions"]}, Options[DiagramsNetGraph]]]
...
DiagramProp[d_, "AbsoluteOptionValue"[opt_], opts : OptionsPattern[]] := OptionValue[{opts, AbsoluteOptions[d], Options[DiagramGraphics], Options[DiagramGrid], Options[DiagramsNetGraph]}, opt]
    ...
    {net, v} |-> Block[{d = AnnotationValue[{net, v}, "Diagram"], wires, out, in, ports, dPorts, portWires, portFunction},
        ...
        If[ Length[portWires] == Length[wires],
            VertexReplace[
                ...
                MapAt[If[FreeQ[#, _Pattern], #, Verbatim[#]] &, {All, 1}] @ First @ Solve[Equal @@@ portWires[[All, All, 2]]]
            ],
            net
        ]
    ],
...
toIdentities[ds : {__Diagram}] := Map[
	Replace[#["HoldExpression"],
		{
			_[Interpretation[_, cycles_Cycles]] :> Splice[IdentityDiagram /@ Thread[PortDual /@ #["InputPorts"] -> Permute[#["OutputPorts"], cycles]]],
            _[Interpretation[_, Identity]] /; #["Arity"] == 0 -> Nothing,
            _ -> #
		}
	] &,
	ds
...
DiagramsGraph[diagrams : {___Diagram ? DiagramQ}, opts : OptionsPattern[]] := Block[{
    ...
    graph = Graph[
        ...
        VertexLabels -> MapAt[Placed[#, Center] &, {All, 2}] @ Join[
            {_ -> Automatic},
            Thread[Range[Length[diagrams]] -> (#["Label"] & /@ diagrams)],
    ...
    embedding = <|
        ...
        If[Catenate[#] === {}, Nothing, With[{diagramCenter = Lookup[embedding, Catenate[#][[1, 1]]]},
            Thread[# -> SortBy[Lookup[embedding, #], - VectorAngle[{1, 0}, (# - diagramCenter)] &, NumericalOrder]] & /@ #
        ]] & /@ indexedPorts
    |>;
    Graph[
        graph,
        FilterRules[{opts}, Options[Graph]],
        VertexCoordinates -> Lookup[embedding, VertexList[graph]]
    ]
]
            ...
            },
            {
                ...
                Switch[edge,
                    _[_, _, {{1, _, _}, ___, {2, _, _}}],
                        ...
                        newSrcPort = TagPort[srcPort, cup];
                        newTgtPort = TagPort[tgtPort, cap];
                    ...
                    _[_, _, {{2, _, _}, ___, {1, _, _}}],
                        ...
                        newSrcPort = TagPort[srcPort, cap];
                        newTgtPort = TagPort[tgtPort, cup];
			...
			Block[{in = Sort @ Cases[EdgeList[graph], _[w_, Verbatim[v], ___] :> w], out = Sort @ Cases[EdgeList[graph], _[Verbatim[v], w_, ___] :> w], wirePorts, sIn, sOut},
				...
				If[ MatchQ[spiders, True | All | Full] && (Length[wirePorts] > 2 ||
					...
					Sow[{v,
                        Which[
                            Length[sIn] == 1,
                                CopyDiagram[#1, #2, "PortArrows" -> #3[[1, 1]]] &,
                            Length[sOut] == 1,
                                MergeDiagram[#2, #1, "PortArrows" -> #3[[2, 1]]] &,
                            Length[sIn] == 0 && Length[sOut] == 2,
                                CupDiagram[#2, "PortArrows" -> #3] &,
                            Length[sOut] == 0 && Length[sIn] == 2,
                                CapDiagram[#1, "PortArrows" -> #3] &,
                            True,
                                SpiderDiagram[#1, #2, "PortArrows" -> #3] &
                        ][
                            If[  #1["DualQ"], With[{x = Unevaluated @@ portFunction[PortDual[#1]]}, tagPort[PortDual[x], {#3}]], With[{x = Unevaluated @@ portFunction[#1]}, tagPort[x, {#3}]]] & @@@ sIn,
                            If[! #1["DualQ"], With[{x = Unevaluated @@ portFunction[PortDual[#1]]}, tagPort[PortDual[x], {#3}]], With[{x = Unevaluated @@ portFunction[#1]}, tagPort[x, {#3}]]] & @@@ sOut,
    ...
    netGraph = Graph[vertices, edges,
        AnnotationRules -> Join[
            Thread[diagramVertices -> List /@ Thread["Diagram" -> KeyValueMap[
                Diagram[#2,
                    "OutputPorts" -> MapThread[With[{makePort = With[{head = If[#1["DualQ"], PortDual, Port]}, With[{x = Unevaluated @@ Port[#1]["TaggedName"]}, head[x, ##2]] &]},
                        FirstCase[edges,
                            edge: DirectedEdge[#2[[1]], tgtIdx_, {{2, _, #2[[3]]}, _, {i_, _, portIdx_}}] :>
                                With[{tgt = diagrams[tgtIdx]["InputOutputPorts", True][[i]][[portIdx]]},
                                   tagPort[makePort[portFunction @ tgt, "NeutralQ" -> tgt["NeutralQ"]], {{tgtIdx, i, portIdx}}]
                                ],
                            tagPort[makePort[portFunction @ #1, "NeutralQ" -> #1["NeutralQ"]], {#2}]
                        ]] &,
                        {#2["OutputPorts"], Thread[{#1, 2, Range[#2["OutputArity"]]}]}
                    ],
                    "InputPorts" -> MapThread[With[{makePort = With[{head = If[#1["DualQ"], PortDual, Port]}, With[{x = Unevaluated @@ Port[#1]["TaggedName"]}, head[x, ##2]] &]},
                        FirstCase[edges,
...
ToDiagramNetwork[d_Diagram, opts : OptionsPattern[]] := Which[
    ...
    ,
    d["SingletonNodeQ"],
    DiagramNetwork[DiagramAssignPorts[d, MapIndexed[TagPort[#1, {#2}] &, d["InputOutputPorts", True], {2}]], FilterRules[{opts}, Options[DiagramNetwork]]]
    ,
    TrueQ[OptionValue["Grid"]],
    DiagramNetwork[
        toDiagramNetwork[If[TrueQ[OptionValue["Arrange"]], d["Arrange", opts], d]["Decompose", "Unary" -> False, "Diagram" -> True], {}, {}, FilterRules[{opts}, Options[toDiagramNetwork]]],
        FilterRules[{opts}, Options[Diagram]],
        d["DiagramOptions"]
    ],
    True,
...
toDiagramNetwork[d_Diagram -> _, pos_, ports_, opts : OptionsPattern[]] := Block[{
    portFunction = OptionValue["PortFunction"],
	...
	Diagram[d,
		...
		"OutputPorts" -> MapIndexed[If[#1["NeutralQ"], #1, With[{p = portFunction[#1]}, TagPort[#1, {If[uniqueQ, Join[pos, {2}, #2], pos]}]]] &, d["OutputPorts"]],
        FilterRules[{opts}, Options[Diagram]]
	]
}
]
...
toDiagramNetwork[CirclePlus[ds___] -> d_, pos_, ports_, opts : OptionsPattern[]] :=
    Catenate @ MapIndexed[toDiagramNetwork[#1, Join[pos, #2], ports, "PortFunction" -> d["PortFunction"], opts] &, {ds}]
...
diagramTensor[diagram_Diagram, opts : OptionsPattern[]] := Replace[diagram["HoldExpression"], {
    ...
    ,
	HoldForm[DiagramDual[d_]] :> Inactive[Conjugate][diagramTensor[d, opts]]
	,
	HoldForm[DiagramFlip[d_]] :> Inactive[ConjugateTranspose][diagramTensor[d, opts], FindPermutation[Catenate[Reverse @ TakeDrop[Range[diagram["Arity"]], diagram["OutputArity"]]]]]
	,
	HoldForm[DiagramReverse[d_]] :> Inactive[Transpose][diagramTensor[d, opts], FindPermutation[Join[Reverse @ Range[diagram["OutputArity"]], Reverse[diagram["OutputArity"] + Range[diagram["InputArity"]]]]]]
	,
	HoldForm[DiagramComposition[ds___]] :> If[TrueQ[OptionValue["ArrayDot"]],
        Fold[Inactive[ArrayDot][If[DiagramQ[#1], diagramTensor[#1, opts], #1], diagramTensor[#2, opts], #2["OutputArity"]] &, {ds}]
        ,
        Dot @@ (
            If[ #["OutputArity"] == #["InputArity"] == 1
                ...
                diagramTensor[#, opts]
                ,
                Inactive[ArrayReshape][diagramTensor[#, opts], {Times @@ Through[#["OutputPorts"]["Name"]], Times @@ Through[#["InputPorts"]["Name"]]}]
            ] & /@ {ds}
	    )
    ]
	,
	HoldForm[DiagramProduct[ds___]] :> If[TrueQ[OptionValue["Kronecker"]],
        ...
        ,
            Inactive[ArrayReshape][
                Inactive[KroneckerProduct] @@ tensors
                ,
        ...
        ,
            If[ AllTrue[tensors, MatchQ[_MatrixSymbol]],
                ...
                With[{indices = FoldPairList[With[{out = #2["OutputArity"], in = #2["InputArity"]}, {#1 + {Range[out], out + Range[in]}, #1 + out + in}] &, 0, {ds}]}, {perm = FindPermutation[Flatten[Thread[indices]]]},
                    If[perm === Cycles[{}], #, Inactive[Transpose][#, perm]] &[Inactive[TensorProduct] @@ tensors]
                ]
            ]
        ]
    ]
	...
	HoldForm[DiagramNetwork[ds___]] :> Block[{
        ...
        tensor = Inactive[TensorContract][
            Inactive[TensorProduct] @@ (diagramTensor[#, opts] & /@ {ds}),
            contraction
        ];
		If[permutation === Cycles[{}], tensor, Inactive[Transpose][tensor, permutation]]
	]
	,
	_ :>
...
DiagramPermute[diagram_ /; diagram["SingletonNodeQ"] || diagram["NetworkQ"], perm_, dualQ : _ ? BooleanQ : True] :=
    Diagram[diagram["Permute", perm, dualQ], "Expression" :> ##] & @@
        Replace[diagram["HoldExpression"], _[Annotation[expr_, opts : OptionsPattern[{"Tensor" -> None}]]] :>
            Hold[Annotation[expr, "Tensor" -> Inactive[Transpose][OptionValue[opts, "Tensor"], perm], FilterRules[{opts}, Except["Tensor"]]]]
        ]
